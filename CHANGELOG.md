...
### Fixes
* Ingestor: the periodic activity announcement is now **opt-in** via a new `ANNOUNCE` env var (default `0`) instead of on by default. It previously shipped with `RX_ONLY` as its only gate, so a stock ingestor deployed to feed a map began broadcasting on its community's default channel unprompted, and the sole opt-out also silenced the MeshCore telemetry polls. `RX_ONLY=1` still forbids every ingestor TX and overrides `ANNOUNCE=1`; the README env table now documents both (SPEC MA7, amended)
