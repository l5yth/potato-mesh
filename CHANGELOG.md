...
### Fixes
* Data: **`PROTOCOL=reticulum` no longer needs `INGESTOR_NODE_ID`** — the ingestor now owns an RNS identity (generated once, stored `0600` as `potato_mesh_identity` inside `RETICULUM_CONFIG_DIR`) and derives its own `!xxxxxxxx` from it through the same identity-hash rule applied to every peer, so the heartbeat and the `reticulum` packets/hour figure come up unaided instead of staying dead until an operator set a variable they had no way to guess. The id is stable across restarts, the variable still overrides it, and the identity is never announced, so nothing new transmits (SPEC RN9)
...
* Web: **a destination hash carried by a stale record was dropped** rather than unioned — the union sat inside the upsert's freshness-guarded `ON CONFLICT` clause, so a second ingestor posting an older announce lost the hash the first had never heard. It is now its own forward-only statement, and still cannot regress `last_heard` or touch a real row from a synthetic placeholder (SPEC RN1)
* Docs: the Reticulum config directory is documented — what it is seeded with, what an `AutoInterface` does and does not reach, and that a scoped ingestor does not inherit your `rnsd`'s interfaces
