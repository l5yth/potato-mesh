...
### Fixes
* **Ingestor: every mesh transmission is now off by default** (`TX_ENABLED`, default `0`). This covers the activity announcement *and* the MeshCore on-air contact telemetry polls — an ingestor deployed to feed a dashboard now transmits nothing until its operator opts in. Announcements need a second, narrower opt-in on top (`TX_ANNOUNCE=1`, default `0`), so permission to transmit is not permission to broadcast unsolicited on a shared human channel. **Breaking for MeshCore operators who want other nodes' telemetry: set `TX_ENABLED=1`.** Companion-link reads over USB/BLE to your own radio are not transmissions and are unaffected (SPEC MA7, amended)
* Ingestor: **`RX_ONLY` no longer fails open.** It was parsed as an exact `== "1"` string comparison, so `RX_ONLY=true`, `TRUE`, `yes`, `on` and `" 1"` all silently resolved to *off* — a receive-only listening post transmitted anyway. All TX flags now accept the common spellings, strip whitespace, and resolve an unrecognized value toward *silence*, warning rather than raising. `RX_ONLY` itself is superseded by `TX_ENABLED` and retired from the docs; it stays supported, and keeps its veto over `TX_ENABLED=1`
* Ingestor: the `TX_*` flags now reach every packaged deployment — `docker-compose.yml`, `data/Dockerfile`, the `flake.nix` NixOS module (`txEnabled`/`txAnnounce`), and `.env.example`. The previous `ANNOUNCE` opt-in reached none of them: the Compose `environment:` mapping is a closed allowlist with no `env_file:`, so setting it in `.env` had no effect at all
* Ingestor: the announcement's 24 h anti-spam delay is measured on a monotonic clock. It used wall clock, so on an RTC-less host (Pi/SBC) a post-boot NTP step cleared the delay instantly and the first announcement fired within a minute of start
* Ingestor: transmit policy is stated in the log at startup without `DEBUG=1`, and each suppressed announcement names the gate that closed (`blocked_by`)
* Docs: README gains a **Transmitting on the mesh** section — what each flag turns on, the truth table, the literal announcement text, and what is never gated. Previously this behavior was visible only in release notes
