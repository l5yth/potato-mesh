...
| # | Decision | Source |
| --- | --- | --- |
| **RN9** | **The ingestor owns a Reticulum identity, so `INGESTOR_NODE_ID` is an override rather than a requirement.** #890 shipped `extract_host_node_id` returning the operator-supplied `INGESTOR_NODE_ID`, on the premise that a passive listener has no handshake revealing "our" node id. The premise holds; the conclusion did not follow. RNS lets any participant hold an identity, so the ingestor generates one on first run, stores it as `potato_mesh_identity` inside the app-owned `RETICULUM_CONFIG_DIR` (RN3), and derives its `!%08x` through **the same identity-hash mapping applied to every peer** (RN1) - so the ingestor's own row is shaped like any other Reticulum row, and no second, private id rule exists to disagree with the first. A plain `PROTOCOL=reticulum` deployment therefore brings up the heartbeat, and so the `reticulum.packets.hour` scope, unaided; `INGESTOR_NODE_ID` still wins when set, which also means no key is generated for a deployment that does not need one. **Persist-or-nothing, deliberately:** an identity that could not be written would be regenerated on every start and each generation is a different node id, so the dashboard would accumulate one orphan ingestor row per restart. The provider therefore reports **no** id in that case - the pre-RN9 behaviour, recoverable - and warns; an existing but unreadable key file is **never** overwritten, since clobbering it would silently change the ingestor's identity, and the likeliest causes (permissions, a half-written file) are ones an operator should resolve deliberately. The key file is written `0600` regardless of umask. **Not a transmit site (extends RN5/MA7):** generating and storing keys is local and the identity is never announced, so `TX_ENABLED=0` remains sufficient. The startup warning #890 emitted for an unset `INGESTOR_NODE_ID` is **removed**: warning about an override nobody must set trains operators to ignore the log. | interview (#888 follow-up) |
