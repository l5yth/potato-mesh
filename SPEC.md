...
| # | Decision | Source |
| --- | --- | --- |
| **AV1** | **Version is the cache key.** Busting is keyed to `PotatoMesh::Application::APP_VERSION`, resolved by `determine_app_version` in precedence order: (1) a non-blank `ENV["APP_VERSION"]` baked into the build (**AV6**), (2) the `git describe --tags --long --abbrev=7` value, (3) `Config.version_fallback` when git metadata is absent. When the version changes, every asset URL changes and browsers refetch. *Documented limitation (narrowed by AV6):* a build with **neither** a baked `APP_VERSION` **nor** git metadata yields the constant fallback version, so an untagged redeploy won't change the buster — the limitation inherent in reusing `APP_VERSION`. **AV6** closes this for the Docker image (the primary deploy mode) by baking the git version in at build time. | interview |
...
| **AV6** | **Baked `APP_VERSION` for git-less builds (Docker).** The published image ships without `.git` (`.dockerignore` excludes it), so `web/Dockerfile` declares `ARG APP_VERSION` → `ENV APP_VERSION` and `.github/workflows/docker.yml` passes `--build-arg APP_VERSION=$(git describe --tags --long --abbrev=7)` computed from its full checkout (to the **web** image only). `determine_app_version` prefers this non-blank ENV, normalized through the same grammar as the git branch so a baked value renders **identically** to a bare-metal checkout at the same commit. This makes the in-image `?v=` buster unique per build, so the `AssetCacheControl` git-gate (**CA-A1**) resolves to `public, max-age=31536000, immutable` inside the image — the full year-long caching win with no stale-JS risk. A plain `docker build` with no build-arg keeps the git/fallback behavior (bounded `max-age=300`). Verified by **CA-A3**. | issue #871 |
