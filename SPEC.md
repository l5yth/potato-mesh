...
| **LV6** | **Per-collection publish cooldown (server-side dedup).** The in-process pub/sub coalesces repeat events for the same collection within a **1 s** window (env-tunable, default 1 s), so N ingestors hearing one packet yield one client refresh/flash, not N. Server-side, in-process (apex-safe, no broker); realises PS4's "short debounce" with an actual time window. | interview |
