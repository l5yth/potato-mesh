...
| **MA7** | **Announcement is quadruple-gated (opt-in).** An announcement is transmitted only when **all** hold: (a) `ANNOUNCE=1` — a dedicated boolean, **default `0`**, and `RX_ONLY` is unset; `RX_ONLY=1` still forbids *every* ingestor TX (the MeshCore polls and the announcement alike) and so overrides `ANNOUNCE`. **Amended:** MA7 originally made `RX_ONLY` the *single* transmit gate with **no separate enable env**, which left the announcement **on by default**. In the field that made every stock ingestor start talking on its community's default channel without its operator having chosen to, and the only opt-out — `RX_ONLY` — also silences the MeshCore telemetry polls, so an operator who wants ingestion but not chatter had no setting for it. Reversed to opt-in: unsolicited TX on a shared human channel is a decision each operator's local community norms must make, not a default (Invariant II, consent). `RX_ONLY` keeps its meaning as the global TX kill switch; (b) the target instance reports **non-private** — the ingestor GETs `<domain>/version` and honors `config.private_mode`, re-checked every cycle, and **fails closed** (skips) on any fetch/parse error, so a privacy signal is never missed (Invariant II); (c) **≥ 24 h have elapsed since ingestor start** — so the first numbers are accurate over a full window and restarts cannot spam the channel. | interview |
