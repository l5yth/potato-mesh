"""Daemon orchestration for synchronising Meshtastic data."""
...
from .packets import _pkt_to_dict, store_packet_dict, upsert_node
...
def on_receive(packet, interface):
    ...
    converted = None
    try:
        converted = _pkt_to_dict(packet)
        store_packet_dict(converted)
    except Exception as exc:
        info: Mapping | type | None
        if isinstance(converted, Mapping):
            info = list(converted.keys())
        else:
            info = type(packet)
...
def _subscribe_receive_topics() -> list[str]:
    """Subscribe :func:`on_receive` to relevant PubSub topics."""
...
def main() -> None:
    ...
    print(
        f"Mesh daemon: nodes+messages → {target} | port={configured_port or 'auto'} | "
        f"channel={config.CHANNEL_INDEX}"
    )
    try:
