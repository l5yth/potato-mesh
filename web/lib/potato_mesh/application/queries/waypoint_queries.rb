...
module PotatoMesh
  module App
    module Queries
      # Fetch waypoints (community POIs, SPEC W1/W4/W5) for the bulk
      # +GET /api/waypoints+ collection.
      #
      # Semantics mirror the other bulk collections: a 7-day rolling window
      # floor on +rx_time+ that callers cannot widen (C4), the BP1 inclusive
      # +before+ cursor for backward pagination, the +KNOWN_PROTOCOLS+-gated
      # protocol filter, and the author opt-out exclusion (W3). Additionally —
      # unique to waypoints — a row whose +expire+ timestamp has passed is
      # excluded from the moment of expiry (W5); +expire+ is stored NULL for
      # never-expiring waypoints, which are served until the window drops them.
      #
      ...
      def query_waypoints(limit, since: 0, before: nil, protocol: nil, now: Time.now.to_i)
        ...
        since_floor = reference_now - PotatoMesh::Config.week_seconds
