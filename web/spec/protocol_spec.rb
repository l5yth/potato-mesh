...
RSpec.describe "Multi-protocol support" do
  ...
  describe "POST /api/ingestors" do
    it "stores protocol when provided" do
      register_ingestor("!11223344", protocol: "meshcore")

      expect(last_response.status).to eq(200)
      with_db(readonly: true) do |db|
        row = db.get_first_row(
          "SELECT protocol FROM ingestors WHERE node_id = ?",
          ["!11223344"],
        )
    ...
    it "updates protocol on re-registration" do
      register_ingestor("!11223344", protocol: "meshtastic")
      register_ingestor("!11223344", protocol: "meshcore")
  ...
  describe "POST /api/nodes with ingestor key" do
    it "inherits protocol from registered ingestor" do
      ...
      with_db do |db|
        db.execute(
          ...
          ["!aabbccdd", 0xaabbccdd, now - 100, now - 200],
        )
      end

      payload = {
        "!aabbccdd" => { "num" => 0xaabbccdd, "lastHeard" => now - 10 },
        "ingestor" => "!11223344",
      }
      ...
      with_db(readonly: true) do |db|
        row = db.get_first_row("SELECT protocol FROM nodes WHERE node_id = ?", ["!aabbccdd"])
        expect(row["protocol"]).to eq("meshcore")
      end
    end
    ...
    it "defaults to meshtastic when ingestor key is absent" do
      with_db do |db|
        db.execute(
          "INSERT INTO nodes(node_id, num, last_heard, first_heard) VALUES(?,?,?,?)",
          ["!ccddee00", 0xccddee00, now - 100, now - 200],
        )
      end

      payload = { "!ccddee00" => { "num" => 0xccddee00, "lastHeard" => now - 10 } }
      post "/api/nodes", payload.to_json, auth_headers
      expect(last_response.status).to eq(200)

      with_db(readonly: true) do |db|
        row = db.get_first_row("SELECT protocol FROM nodes WHERE node_id = ?", ["!ccddee00"])
        expect(row["protocol"]).to eq("meshtastic")
      end
    end
  end
  ...
  describe "GET ?protocol= filter" do
    before do
      ...
      with_db do |db|
        db.execute(
          ...
          ["!mesh0001", 1, now - 10, now - 20, "meshtastic"],
        )
        db.execute(
          "INSERT INTO nodes(node_id, num, last_heard, first_heard, protocol) VALUES(?,?,?,?,?)",
          ["!core0001", 2, now - 10, now - 20, "meshcore"],
        )
        db.execute(
    ...
    it "filters /api/nodes by protocol" do
      ...
      expect(ids).to include("!core0001")
      expect(ids).not_to include("!mesh0001")
    end
    ...
    it "filters /api/ingestors by protocol" do
      with_db do |db|
        db.execute(
          ...
          ["!mesh9999", now - 60, now, "0.5.11", "meshtastic"],
        )
      end
      ...
      expect(last_response.status).to eq(200)
      ids = JSON.parse(last_response.body).map { |r| r["node_id"] }
      expect(ids).to include("!11223344")
      expect(ids).not_to include("!mesh9999")
    end
    ...
    it "returns all records when protocol param is absent" do
      ...
      expect(last_response.status).to eq(200)
      ids = JSON.parse(last_response.body).map { |r| r["node_id"] }
      expect(ids).to include("!mesh0001")
      expect(ids).to include("!core0001")
    end
